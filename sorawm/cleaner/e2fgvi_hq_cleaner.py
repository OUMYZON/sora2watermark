...
def get_ref_index(frame_idx: int, neighbor_ids: List[int], length: int, ref_length: int, num_ref: int) -> List[int]:
...
class E2FGVIHDCleaner:
    ...
    def __init__(self, ckpt_path: Path=E2FGVI_HQ_CHECKPOINT_PATH, config: E2FGVIHDConfig=E2FGVIHDConfig()):
    ...
    def process_frames_chunk(self, chunk_length: int, neighbor_stride: int, \
        imgs_chunk: torch.Tensor, masks_chunk: torch.Tensor, binary_masks_chunk: np.ndarray, \
        frames_np_chunk: np.ndarray, h: int, w: int) -> List[np.ndarray]:
        comp_frames_chunk = [None] * chunk_length

        for f in tqdm(range(0, chunk_length, neighbor_stride), desc=f"  Frame progress", position=1, leave=False):
            neighbor_ids = [
                i for i in range(max(0, f - neighbor_stride),
                            min(chunk_length, f + neighbor_stride + 1))
            ]
            ref_ids = get_ref_index(f, neighbor_ids, chunk_length, self.config.ref_length, self.config.num_ref)
            ...
            with torch.no_grad():
                ...
                for i in range(len(neighbor_ids)):
                    idx = neighbor_ids[i]
                    img = np.array(pred_imgs[i]).astype(np.uint8) * binary_masks_chunk[idx] + \
                        frames_np_chunk[idx] * (1 - binary_masks_chunk[idx])
                    if comp_frames_chunk[idx] is None:
                        comp_frames_chunk[idx] = img
                    else:
                        comp_frames_chunk[idx] = comp_frames_chunk[idx].astype(np.float32) * 0.5 + \
    ...
    def clean(self, frames: np.ndarray, masks: np.ndarray) -> List[np.ndarray]:
        ...
        logger.debug(f'Processing {video_length} frames in {num_chunks} chunks (chunk_size={chunk_size}, overlap={overlap_size})')
        ...
        for chunk_idx in tqdm(range(num_chunks), desc='Chunk', position=0, leave=True):
            ...
            comp_frames_chunk = self.process_frames_chunk(
                actual_chunk_size, self.config.neighbor_stride,
                imgs_chunk, masks_chunk, binary_masks_chunk,
                frames_np_chunk, h, w
            )
            ...
            if chunk_idx == 0:
                ...
                for i in range(overlap_start, overlap_end):
                    if comp_frames[start_idx + i] is not None and comp_frames_chunk[i] is not None:
                        alpha = i / overlap_end
